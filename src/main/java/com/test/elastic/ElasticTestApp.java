...
public class ElasticTestApp {
	...
	public void node_command(int nodeid,Node node, String updown) throws IOException 
	{
		...
		if (updown.equals("DOWN"))
		{
			...
			command="sudo -S -p '' pgrep -f /var/lib/"+node.getHome_path()+" | xargs kill -9";
		}
		else
		if (updown.equals("UP"))
		{
	...
	public void cluster_check() throws Exception
	{
		...
		int upnode=0;
			...
			for(int i=0;i<nodes.length;i++)
			{
				...
				{
					//get cluster health
					String LIST_NODES = "http://"+nodes[i].getIp()+":"+nodes[i].getPort()+"/_nodes/node"+(i+1);
					...
					{
						upnode=(i+1);
						System.out.println("in if"+upnode);
					}
				}
